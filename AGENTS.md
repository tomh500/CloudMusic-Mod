...
- 新增：**歌词样式三态** `/cloudmusic lyric [default|actionbar|off]`（`Configs.GUI.LYRIC_STYLE`，`config/LyricStyle.java` 枚举）。`actionbar` 用原版 `Hud.setOverlayMessage(...)`（即 `/title actionbar` 样式）每帧刷新居中显示当前歌词；`off` 关闭。原来的 `lyric` 布尔配置项已删除，旧配置里遗留的 `cloudmusic.config.lyric` 键会被 malilib 忽略。
- 新增：**聊天栏打开时可用鼠标拖动歌词和歌曲信息面板**，松手后位置写入 malilib 配置 `music.info.x/y`、`lyric.x/y`，悬停时显示白色边框 + 移动光标。
- 新增：**聊天栏里可拖动进度条快速跳转**（`MusicPlayer.seek(ms)`：重新打开播放源后**读取并丢弃 PCM 字节**精确定位，见下）。拖动中橙色进度会跟着鼠标预览，松手才真正 seek。
- 新增：**点击专辑封面切换播放/暂停**（聊天栏打开时，封面区域 `DragTarget.COVER`，松手调 `MusicPlayer.switchPlay()`，悬停显示手型光标）。
...
- `render/MusicHudRenderer.java`（新增）：HUD 三块——歌曲信息面板（封面+文字+进度条）、歌词、登录二维码。还负责聊天栏拖拽：`handleDrag()` 每帧轮询（仅 `client.gui.screen() instanceof ChatScreen` 时生效，26.2 当前屏幕在 `Gui` 上，不是 `Minecraft.screen`），拖拽目标分 `COVER / MUSIC_INFO / LYRIC / PROGRESS`（封面优先，松手时 `switchPlay()`；进度条优先，松手时 `seek`），按键状态用 `GLFW.glfwGetMouseButton`（`MouseHandler.isLeftPressed` 在屏幕打开时不更新）；`renderDragHints()` 画边框 + `requestCursor`（封面 `POINTING_HAND`，面板/歌词 `RESIZE_ALL`，进度条 `RESIZE_EW`）；松手 `Configs.INSTANCE.save()`；歌词 `actionbar` 样式在 `renderLyricActionBar()` 里每帧 `setOverlayMessage`；`isUiHidden()` 在非聊天界面时隐藏全部控件。
...
- **seek 实现（已修）**：`MusicPlayer.seek(ms)` 只设 `seekTargetMs`（volatile），播放循环检测到后关闭当前流、`openAudioInputStream()` 重开（本地文件 `playFile` 或 URL `playUrl`），再 `startPlayingTime = now - target` 继续。**坑：不能用 `stream.skip()`**——mp3spi 等解码后的转换流上 `skip()` 按压缩源字节跳过，会越过目标位置直接 EOF（实测要 60s 的字节数 skip 后首读返回 0/-1），表现为拖动进度条后橙色不刷新 + 当前歌曲提前结束自动切歌。现在 `seekStream()` 从头部读取并丢弃 PCM 字节（64KB 缓冲，60s 跳转约 250ms），seek 后 `play.flush()` 丢弃旧缓冲。歌词线程每 50ms 轮询进度，往回调时从头部重找当前行（`index < 0 || time < entries.get(index).getKey()` 分支）。
